...
public class Bearbeitenmodus {
	...
	private TableColumn<Card, String> ersteSprache;
	private TableColumn<Card, String> zweiteSprache;
	private TableColumn<Card, Boolean> doppelt;
	...
	public Tab buildBearbeitenGui() {
		...
		ersteSprache.setCellFactory(TextFieldTableCell.<Card> forTableColumn());
		zweiteSprache
				.setCellFactory(TextFieldTableCell.<Card> forTableColumn());
		
		/*doppelt.setCellFactory(new Callback<TableColumn<Card, Boolean>, TableCell<Card,Boolean>>() {

			@Override
			public TableCell<Card, Boolean> call(TableColumn<Card, Boolean> arg0) {
				return new CheckBoxTableCell<Card,Boolean>();
			}
		...
		//doppelt.setResizable(false);
		
		ersteSprache
				.setCellValueFactory(new PropertyValueFactory<Card, String>(
						"question") // Das sind die Felder in Card
				);
		zweiteSprache
				.setCellValueFactory(new PropertyValueFactory<Card, String>(
						"anwser"));
		/*doppelt.setCellValueFactory(new PropertyValueFactory<Card, Boolean>(
				"doppelt")); */

		HBox hbox = new HBox(16);
		hbox.setAlignment(Pos.CENTER); // TODO: hier noch doppelt checkbox
										// hinmachen
		hbox.setPadding(new Insets(0, 5.3, 0, 5.3));
		Label question = new Label("Question:");
		questionField = new TextField();
		Label awnser = new Label("Anwser:");
		...
		hbox.getChildren().addAll(question, questionField, awnser, awnserField,
				hinzufuegen);
		...
		public void handle(ActionEvent e) {
			if (e.getSource() == laden) {
				...
				if (file != null) {
					...
					int index = name.lastIndexOf("."); // Endung entfernen
														// (falls vorhanden)
					if (index > 0) {
				...
				table.getItems().add(
						new Card(questionField.getText(),
								awnserField.getText(), false));
				questionField.clear();
				awnserField.clear();
				sortTable();
	...
	public class columnHandler implements
			EventHandler<CellEditEvent<Card, String>> {
		public void handle(CellEditEvent<Card, String> e) {
			if (e.getTableColumn().equals(ersteSprache)) {
				((Card) e.getTableView().getItems()
						.get(e.getTablePosition().getRow())).setQuestion(e
						.getNewValue());
			} else {
				((Card) e.getTableView().getItems()
						.get(e.getTablePosition().getRow())).setAnwser(e
						.getNewValue());
			}
			sortTable();
			speichern.setDisable(false);
	...
	public class BooleanColumnHandler implements EventHandler<CellEditEvent<Card, Boolean>> {
		public void handle(CellEditEvent<Card, Boolean> e) {
			e.getTableView().getItems().get(e.getTablePosition().getRow()).setDoppelt(e.getNewValue());
			speichern.setDisable(false);
		}
	}
